...
Avia 提供两个入口，配置校验、节点名 `livox_avia_publisher` 和四个公开参数完全
相同，区别仅是固定的点云格式：
...
应看到节点 `livox_avia_publisher`。`avia.launch.py` 的点云类型为
`livox_interfaces/msg/CustomMsg`；`avia_pointcloud2.launch.py` 的点云类型为
`sensor_msgs/msg/PointCloud2`，字段布局为 Livox PointXYZRTL：
`x`、`y`、`z`、`intensity` 是 `float32`，`tag`、`line` 是 `uint8`。两个入口
的 IMU 类型都是 `sensor_msgs/msg/Imu`。

零坐标点会保留原始索引和时间结构，下游必须显式过滤 `x=y=z=0`。PointCloud2
没有逐点时间，建议用于避障/可视化；LIO 使用 CustomMsg 的
`timebase + offset_time`。IMU 线加速度已转换为 `m/s²`，不可用姿态以
`orientation_covariance[0] = -1` 标记。默认 `timesync=false` 时 Header 是设备
上电后的相对时间；与外部传感器融合仍需单独完成时间同步。
